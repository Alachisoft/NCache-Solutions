    ...
    public class PersistenceWriteThruProvider : ProviderBase, IWriteThruProvider
    {
        ...
        public OperationResult WriteToDataSource(WriteOperation operation)
        {
            ...
            try
            {
                switch (operation.OperationType)
                {
                    ...
                    case WriteOperationType.Update:
                        ...
                        _persistenceProvider.Insert(itemsToUpdate);
                        break;

                    case WriteOperationType.Delete:
                        _persistenceProvider.Remove(new string[] { operation.Key });
                        break;
            ...
            catch (Exception ex)
            {
                ...
                operationResults = new OperationResult(operation, OperationResult.Status.Success);
            }
                ...
                try
                {
                    switch (dtOperation.OperationType)
                    {
                        ...
                        case DatastructureOperationType.CreateDataType:
                            ...
                            _persistenceProvider.Insert(dict);
                            break;

                        case DatastructureOperationType.DeleteDataType:
                            _persistenceProvider.Remove(new string[] { dtOperation.Key });
                            break;

                        case DatastructureOperationType.AddToDataType:
                            
                            _persistenceProvider.AddToDataType(dtOperation.Key, dtOperation.ProviderItem, dtOperation.DataType);
                            break;

                        case DatastructureOperationType.UpdateDataType:
                            _persistenceProvider.UpdateToDataType(dtOperation.Key, dtOperation.ProviderItem, dtOperation.DataType);
                            break;

                        case DatastructureOperationType.DeleteFromDataType:
                            _persistenceProvider.RemoveFromDataType(dtOperation.Key, dtOperation.DataType);
                            break;
                    } 
