...
namespace Microsoft.eShopOnContainers.WebMVC
{
    ...
    static class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddHealthChecks(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (configuration.GetValue<string>("IsNCacheClusterEnv") == bool.TrueString)
            {
                var cacheID = "mvccache";
                if (!string.IsNullOrEmpty(configuration["CacheID"]))
                {
                    cacheID = configuration["CacheID"];
                }

                var ipAddresses = configuration.GetValue<string>("MVCCacheIPAddresses", "").Split('-');

                if (ipAddresses.Length == 0)
                {
                    throw new ArgumentNullException("No IP addresses given for mvc cache");
                }
                ...
                builder.AddNCacheHealthCheck(configuration["CacheID"], 
                    name: "webmvc-ncache-check", 
